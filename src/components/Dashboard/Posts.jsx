...
export const Posts = ({ post }) => {
  ...
  const handleCelebration = () => {
    setActions(
      isCelebrated
        ? { ...actions, celebrate: actions.celebrate - 1 }
        : { ...actions, celebrate: actions.celebrate + 1 }
    );
    setIsCelebrated(!isCelebrated);
  };

  //handle visibility of comments
    ...
    <section className="w-full bg-white rounded mt-4 pt-7 pb-5 sm:px-10 px-4">
      <div className="flex justify-between">
        <div className="flex gap-x-3">
          <UserAvatar src={post.photo} alt={post.name + "ggt avatar"} />
          <UserPostDetails
            user_name={post.name}
        ...
        <div>
          ...
          <div
            id="dropdownDots"
            className={`ml-[-150px] absolute z-10 float-left ${
      ...
      <div className="flex gap-x-5 pt-2 items-center text-sm mt-2">
        ...
        <button
          ...
          className="flex gap-x-1 px-3 py-3 hover:bg-gray-100 rounded"
        >
          <SlLike className="like-icon-btn text-lg" />
          Like
        </button>
        <div
          className={`ml-[-5px] mt-[-85px] absolute z-10 float-left ${
            showEmoji ? "block" : "hidden"
          } bg-white divide-y divide-gray-100 rounded shadow px-3 py-3 w-40`}
        >
          <div className="flex justify-between">
      ...
      {showComments && <Comment comments={post.comments.comments} />}
    </section>
  );
};
