...
export const AddPost = (props) => {
  ...
  const handleAddLocation = () => {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      fetch(
        `https://api.radar.io/v1/geocode/reverse?coordinates=${lat},${lgt}`,
        {
        ...
        .then((data) => {
          setCurrentLocation(
            `${data.addresses[0].city}, ${data.addresses[0].country}`
          );
        })
  ...
  return (
    <section className="w-full bg-white pt-2.5 pb-1 rounded mt-4 sm:mt-0">
          ...
          <div className="flex gap-x-2.5">
            ...
            {currentLocation && (
              <div className="text-sm flex mx-auto leading-3 gap-x-1 text-[#01B0F1]">
                <BsFillGeoAltFill className="text-sm text-[#01B0F1]" />{" "}
                {currentLocation}
                <DeleteUpload
                  top="top-[-10px]"
                  right="right-35px]"
      ...
      <div className="w-full bg-white flex justify-between shrink border-t-[0.5px] border-solid border-[rgba(128, 122, 122, 0.5)] py-2.5 px-8">
        <div className="flex gap-x-7 my-auto">
          <button
            className="flex gap-x-1.5 bg-none rounded-0"
            onClick={handlePhotoUpload}
          >
            <MdInsertPhoto className="text-xl text-[#01B0F1]" />
            <div className="text-xs text-gray-400  leading-5  hidden sm:block">
              Photo
            </div>
          </button>
        ...
        <div>
          <button
            className="text-white bg-[#01B0F1] rounded py-2.5 px-10 text-xs"
